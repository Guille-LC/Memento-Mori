    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center px-4">
      <section className=" w-full max-w-[1100px] bg-zinc-900 border border-zinc-800 rounded-xl p-8 shadow-2xl space-y-6">
        ...
        <div className='flex justify-center'>
          <button onClick={handleCalculate} className="text-sm underline text-zinc-300 text-center">
        ...
        {/* Resumen de vida */}
        {birthDate && (
          <div className="space-y-2 text-sm text-zinc-300 text-center">
            <p>
            ...
            <div className="mt-8 flex justify-center">
              <div className="grid gap-[3px]" style={{gridTemplateColumns: 'repeat(52, 1fr)',}}>
                ...
                {Array.from({ length: totalWeeks }).map((_, index) => {
                  ...
                  return <div key={index} className={` w-[8px] h-[8px] rounded-[1px] ${color} ${weekOfYear !== 0 && weekOfYear % 13 === 0 ? 'ml-0.5' : ''} ${year % 10 === 0 ? 'mt-2' : ''}`}></div>
                })}
              </div>
            </div>
          </>
        ...
        <h2 className="flex items-center gap-2 text-sm uppercase tracking-widest text-zinc-400 text-center justify-center">
          MEMENTO MORI <Image src="/omega.png" alt="omega" width={20} height={20} className="opacity-90 invert"/>
        </h2>
